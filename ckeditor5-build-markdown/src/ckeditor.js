...
MarkdownEditor.builtinPlugins = [
	...
	function (editor) {
		...
		// Enhanced code block behavior
		editor.editing.view.document.on('keydown', (evt, data) => {
			const selection = editor.model.document.selection;
			const position = selection.getFirstPosition();
			
			// Only handle Enter key in code blocks
			if (data.keyCode === 13 && position && position.parent && position.parent.name === 'codeBlock') { 
				// Allow normal Enter behavior in code blocks for line breaks
				// CKEditor will handle this properly by default
				return;
			}
			
			// Handle arrow keys to exit code blocks
			if ((data.keyCode === 40 || data.keyCode === 38) && position && position.parent && position.parent.name === 'codeBlock') { // Down or Up arrow
				const codeBlock = position.parent;
				const isAtEnd = position.offset === codeBlock.maxOffset;
				const isAtStart = position.offset === 0;
				
				if ((data.keyCode === 40 && isAtEnd) || (data.keyCode === 38 && isAtStart)) {
					// Let CKEditor handle navigation out of code blocks
					return;
				}
			}
		}, { priority: 'normal' });
